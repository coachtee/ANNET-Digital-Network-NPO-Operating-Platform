...
3. **Official logo artwork.** No vector or high-resolution logo file was ever supplied — only the approved mockup screenshot, itself a compressed raster image. The favicon (`static/img/favicon*.{ico,png}`) was extracted directly from the mockup's logo emblem — legible at favicon size, but the crop is only 80×80px natively and too low-resolution to use anywhere larger (the header/sidebar "AN" mark stays as a clean CSS badge in the brand colour for that reason, rather than a blurry raster swap). Obtain the real SVG/high-res logo from Naleli Innovations / ANNET and swap it into `templates/partials/public_header.html`, `templates/workspace_base.html` and the favicon set before public launch.
4. **Scheduled reminders** (compliance deadlines, policy review dates, governance term expiry) — the data model supports it (`ComplianceObligation.due_date`, `Policy.next_review_date`, `GovernanceOfficial.term_end`) but no `send_reminders` management command exists yet. Straightforward to add per `DEPLOYMENT.md` section 7 — do this next if reminders are a launch requirement.
5. **Enhanced access control for sensitive beneficiary records.** `Beneficiary.is_sensitive` exists as a flag but is not yet enforced by an additional permission gate beyond the standard `beneficiaries.view`/`beneficiaries.manage` capabilities. Add a dedicated capability (e.g. `beneficiaries.view_sensitive`) if/when a real sensitive-service programme is onboarded.
6. **Rate limiting** on login/password-reset — not yet added (spec section 38, "where practical"). `django-ratelimit` or equivalent is the straightforward path; Redis (now available in the Docker deployment) is the natural backing store.
7. **Governance meeting minutes linking UI.** `GovernanceMeeting.minutes_document` exists on the model but the meeting detail template only links out to the general document vault — no in-page "attach this document as minutes" control yet.
8. **Funder Workspace** (spec section 35) — explicitly a later phase per the spec; no code exists for it. `Grant.responsible_manager` and the existing capability framework are the natural extension point when this is prioritised.
9. **CI pipeline** — no GitHub Actions workflow runs `manage.py test` on push yet. Recommended before merging further feature branches at scale.
10. **Opportunity → organisation matching** (spec section 34) — `Opportunity.target_sectors`/`target_provinces` fields exist but nothing consumes them yet; explicitly a "future functionality" item in the spec.
